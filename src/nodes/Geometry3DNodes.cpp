...
namespace
{
   ...
   const char* kFragSrc =
      ...
      "}\n"
      "void getCotangentFrame(vec3 normal, vec3 pos, vec2 uv, out vec3 tangent, out vec3 bitangent) {\n"
      "   vec3 dp1 = dFdx(pos);\n"
      "   vec3 dp2 = dFdy(pos);\n"
      "   vec2 duv1 = dFdx(uv);\n"
      "   vec2 duv2 = dFdy(uv);\n"
      ...
      "   } else {\n"
      "      tangent = abs(normal.z) < 0.999 ? normalize(cross(normal, vec3(0.0, 0.0, 1.0))) : normalize(cross(normal, vec3(1.0, 0.0, 0.0)));\n"
      "      bitangent = normalize(cross(normal, tangent));\n"
      "   }\n"
      "}\n"
      "vec3 applyNormalMap(vec3 normal, vec2 uv) {\n"
      ...
      "   if (uHasNormalMap == 1) n = applyNormalMap(n, mapUv);\n"
      ...
      "\n"
      "   vec3 tangent, bitangent;\n"
      "   getCotangentFrame(n, vWorldPos, mapUv, tangent, bitangent);\n"
      "   if (abs(uAnisotropy) > 0.001) {\n"
      "      float rotRad = uAnisotropyRotation * 6.2831853;\n"
      "      vec3 tRot = normalize(tangent * cos(rotRad) + bitangent * sin(rotRad));\n"
      "      bitangent = normalize(cross(n, tRot));\n"
      "      tangent = tRot;\n"
      "   }\n"
      "\n"
      "   for (int i = 0; i < 3; i++) {\n"
