...
const std::vector<FieldSynthNode::Preset>& FieldSynthNode::Presets()
{
   static const std::vector<Preset> kPresets = {
      ...
      { "Polyphonic FM Bell",
        ...
        "mIdx = modIndex * (env * brightness + (1.0 - brightness) * 0.3)\n"
        "modIn = pMod + (lastMod * feedback) * 0.159155\n"
        "modSig = sin(6.2831853 * modIn) * mIdx\n"
        "lastMod = modSig\n\n"
        "pMod = (pMod + (freq * modRatio) / sr) % 1.0\n"
        "pCarrier = (pCarrier + freq / sr) % 1.0\n"
        "carrierPhase = 6.2831853 * pCarrier + modSig\n"
        "out = sin(carrierPhase) * env\n" },
      ...
      { "Acid Square Bass",
        ...
        "inSig = sq * drive\n"
        "dist = inSig / (1.0 + abs(inSig) * 0.4)\n\n"
        "fCut = clamp(cutoff + env * accent * 4000.0, 40.0, sr * 0.40)\n"
        "f = clamp((fCut / sr) * 2.5, 0.005, 0.42)\n"
        "fb = (lp2 * res * 3.5) / (1.0 + abs(lp2) * 1.5)\n"
        "u = dist - fb\n"
        "satU = u / (1.0 + abs(u) * 0.2)\n"
        "lp1 = lp1 + f * (satU - lp1)\n"
