...
const std::vector<FieldSynthNode::Preset>& FieldSynthNode::Presets()
{
   static const std::vector<Preset> kPresets = {
      ...
      { "Acoustic Trap Snare",
        ...
        "seed = (seed * 11035.152 + 0.12345) % 1.0\n"
        "white = seed * 2.0 - 1.0\n"
        "hp = hp + 0.25 * (white - hp)\n"
      ...
      { "Acoustic Grand Piano",
        ...
        "strike = if(noteOn > 0.5, noteVel * hammer, strike * 0.88)\n\n"
        ...
        "cosW1 = cos(w1)\n"
        "sinW1 = sin(w1)\n"
        "n1 = (s1 * cosW1 - s2 * sinW1 + strike) * dCoeff\n"
        "n2 = (s1 * sinW1 + s2 * cosW1) * dCoeff\n"
        "s1 = n1 / (1.0 + abs(n1) * 0.01)\n"
        "s2 = n2 / (1.0 + abs(n2) * 0.01)\n\n"
        "cosW2 = cos(w2)\n"
        "sinW2 = sin(w2)\n"
        "n3 = (s3 * cosW2 - s4 * sinW2 + strike * 0.4 * brightness) * dCoeff * 0.996\n"
        "n4 = (s3 * sinW2 + s4 * cosW2) * dCoeff * 0.996\n"
        "s3 = n3\n"
        "s4 = n4\n\n"
      ...
      { "Crystalline Tubular Bell",
        ...
        "state float c1 = 0.0\n"
        "state float c2 = 0.0\n\n"
        "hammer = if(noteOn > 0.5, noteVel * strike, hammer * 0.85)\n\n"
        "w1 = min(6.283185 * clamp(freq, 40.0, sr * 0.42) / sr, 2.8)\n"
        "w2 = min(w1 * 2.76, 2.8)\n"
        "w3 = min(w1 * 5.40, 2.8)\n\n"
        "dc1 = clamp(1.0 - 1.0 / (sr * max(decay, 0.5)), 0.98, 0.999999)\n"
        "dc2 = dc1 * 0.999\n"
        "dc3 = dc2 * 0.998\n\n"
        "cw1 = cos(w1)\n"
        "sw1 = sin(w1)\n"
        "na1 = (a1 * cw1 - a2 * sw1 + hammer) * dc1\n"
        "na2 = (a1 * sw1 + a2 * cw1) * dc1\n"
        "a1 = na1\n"
        "a2 = na2\n\n"
        "cw2 = cos(w2)\n"
        "sw2 = sin(w2)\n"
        "nb1 = (b1 * cw2 - b2 * sw2 + hammer * 0.5) * dc2\n"
        "nb2 = (b1 * sw2 + b2 * cw2) * dc2\n"
        "b1 = nb1\n"
        "b2 = nb2\n\n"
        "cw3 = cos(w3)\n"
        "sw3 = sin(w3)\n"
        "nc1 = (c1 * cw3 - c2 * sw3 + hammer * 0.25) * dc3\n"
        "nc2 = (c1 * sw3 + c2 * cw3) * dc3\n"
        "c1 = nc1\n"
        "c2 = nc2\n\n"
        "out = (a1 + b1 * 0.6 + c1 * 0.3) * 0.6\n" },
      { "Music Box Celesta",
      ...
      { "Raindrops & Water Drips",
        ...
        "seed = (seed * 11035.152 + 0.12345) % 1.0\n"
        "dropFreq = if(isNewDrop > 0.5, 600.0 + seed * 1600.0, dropFreq * 1.0003)\n"
      ...
      { "Campfire Fire Crackle",
        ...
        "state float seed = 0.8234\n"
        "state float pop = 0.0\n"
        "state float hiss = 0.0\n\n"
        "seed = (seed * 11035.152 + 0.12345) % 1.0\n"
        "isPop = if(seed < crackleDensity, 1.0, 0.0)\n"
        "pop = if(isPop > 0.5, (seed / crackleDensity) * 1.8 - 0.9, pop * 0.92)\n\n"
        "white = seed * 2.0 - 1.0\n"
