...
const std::vector<FieldSampleNode::Preset>& FieldSampleNode::Presets()
{
   static const std::vector<Preset> kPresets = {
      ...
      { "Sidechain Ducker",
        ...
        "out = in * gain\n" },
      { "Analog Chorus",
        "param float rate = 0.6 [0.05, 5.0]\n"
        "param float depthMs = 6.0 [0.5, 15.0]\n"
        "param float mix = 0.5 [0.0, 1.0]\n"
        "state float buf[3000] = 0.0\n"
        "state float wpos = 0.0\n"
        "state float phase = 0.0\n\n"
        "phase = (phase + rate / sr) % 1.0\n"
        "lfo = sin(6.283185 * phase)\n\n"
        "baseMs = 15.0\n"
        "delaySamples = clamp((baseMs + lfo * depthMs) * 0.001 * sr, 4.0, 2990.0)\n"
        "rpos = (wpos - delaySamples + 6000.0) % 3000.0\n"
        "i0 = floor(rpos)\n"
        "frac = rpos - i0\n"
        "a = buf[i0]\n"
        "b = buf[(i0 + 1.0) % 3000.0]\n"
        "readVal = a + (b - a) * frac\n\n"
        "buf[wpos] = in\n"
        "wpos = (wpos + 1.0) % 3000.0\n\n"
        "out = in * (1.0 - mix) + readVal * mix\n" },
      { "Resonant Flanger",
        "param float rate = 0.25 [0.02, 3.0]\n"
        "param float depthMs = 2.5 [0.2, 6.0]\n"
        "param float feedback = 0.5 [0.0, 0.95]\n"
        "param float mix = 0.5 [0.0, 1.0]\n"
        "state float buf[600] = 0.0\n"
        "state float wpos = 0.0\n"
        "state float phase = 0.0\n\n"
        "phase = (phase + rate / sr) % 1.0\n"
        "lfo = 0.5 + 0.5 * sin(6.283185 * phase)\n\n"
        "baseMs = 0.3\n"
        "delaySamples = clamp((baseMs + lfo * depthMs) * 0.001 * sr, 1.0, 590.0)\n"
        "rpos = (wpos - delaySamples + 1200.0) % 600.0\n"
        "i0 = floor(rpos)\n"
        "frac = rpos - i0\n"
        "a = buf[i0]\n"
        "b = buf[(i0 + 1.0) % 600.0]\n"
        "readVal = a + (b - a) * frac\n\n"
        "buf[wpos] = in + readVal * feedback\n"
        "wpos = (wpos + 1.0) % 600.0\n\n"
        "out = in * (1.0 - mix) + readVal * mix\n" },
      { "Analog Phaser",
        "param float rate = 0.3 [0.02, 3.0]\n"
        "param float depth = 0.8 [0.0, 1.0]\n"
        "param float feedback = 0.3 [0.0, 0.9]\n"
        "param float mix = 0.5 [0.0, 1.0]\n"
        "state float phase = 0.0\n"
        "state float fb = 0.0\n"
        "state float x1a = 0.0\n"
        ...
        "phase = (phase + rate / sr) % 1.0\n"
        "lfo = 0.5 + 0.5 * sin(6.283185 * phase)\n"
        "phFreq = 200.0 + lfo * depth * 2200.0\n\n"
        "wc = 3.14159 * clamp(phFreq, 50.0, sr * 0.45) / sr\n"
        "ww = sin(wc) / cos(wc)\n"
        "a = (1.0 - ww) / (1.0 + ww)\n\n"
        "xin = in + fb * feedback\n\n"
        "ya = -a * xin + x1a + a * y1a\n"
        ...
        "out = in * (1.0 - mix) + wet * mix\n" },
      { "Plate Reverb",
        "param float size = 0.6 [0.1, 1.0]\n"
        "param float damp = 0.4 [0.0, 1.0]\n"
        "param float decay = 0.75 [0.1, 0.97]\n"
        "param float mix = 0.35 [0.0, 1.0]\n"
        "state float c1[1687] = 0.0\n"
        "state float c2[1601] = 0.0\n"
        "state float c3[2053] = 0.0\n"
        "state float wc1 = 0.0\n"
        "state float wc2 = 0.0\n"
        "state float wc3 = 0.0\n"
        "state float lp1 = 0.0\n"
        "state float lp2 = 0.0\n"
        "state float lp3 = 0.0\n"
        "state float a1[225] = 0.0\n"
        "state float a2[341] = 0.0\n"
        "state float wa1 = 0.0\n"
        "state float wa2 = 0.0\n\n"
        "len1 = clamp(floor(1687.0 * size), 200.0, 1687.0)\n"
        "len2 = clamp(floor(1601.0 * size), 200.0, 1601.0)\n"
        "len3 = clamp(floor(2053.0 * size), 200.0, 2053.0)\n\n"
        "i1 = wc1 % len1\n"
        "y1 = c1[i1]\n"
        "lp1 = lp1 + (1.0 - damp) * (y1 - lp1)\n"
        "c1[i1] = in + lp1 * decay\n"
        "wc1 = (wc1 + 1.0) % 1687.0\n\n"
        "i2 = wc2 % len2\n"
        "y2 = c2[i2]\n"
        "lp2 = lp2 + (1.0 - damp) * (y2 - lp2)\n"
        "c2[i2] = in + lp2 * decay\n"
        "wc2 = (wc2 + 1.0) % 1601.0\n\n"
        "i3 = wc3 % len3\n"
        "y3 = c3[i3]\n"
        "lp3 = lp3 + (1.0 - damp) * (y3 - lp3)\n"
        "c3[i3] = in + lp3 * decay\n"
        "wc3 = (wc3 + 1.0) % 2053.0\n\n"
        "combSum = (y1 + y2 + y3) * 0.33\n\n"
        "g = 0.5\n"
        "r1 = a1[wa1]\n"
        "ap1 = -g * combSum + r1\n"
        "a1[wa1] = combSum + g * r1\n"
        ...
        "a2[wa2] = ap1 + g * r2\n"
        "wa2 = (wa2 + 1.0) % 341.0\n\n"
        "out = in * (1.0 - mix) + ap2 * mix\n" },
      { "Noise Gate",
        "param float threshold = 0.08 [0.005, 0.5]\n"
        "param float ratio = 8.0 [2.0, 40.0]\n"
        "param float attack = 0.003 [0.0005, 0.05]\n"
        "param float release = 0.15 [0.01, 1.0]\n"
        "state float env = 0.0\n"
        "state float gain = 1.0\n\n"
        "lvl = abs(in)\n"
        "attCoef = exp(-1.0 / (max(attack, 0.0001) * sr))\n"
        "relCoef = exp(-1.0 / (max(release, 0.0001) * sr))\n"
        "env = if(lvl > env, env * attCoef + lvl * (1.0 - attCoef), env * relCoef + lvl * (1.0 - relCoef))\n\n"
        "below = clamp((threshold - env) / max(threshold, 0.0001), 0.0, 1.0)\n"
        "targetGain = 1.0 / (1.0 + below * (ratio - 1.0))\n\n"
        "gain = gain + 0.05 * (targetGain - gain)\n"
        "out = in * gain\n" },
      { "Wavefolder",
        "param float drive = 3.0 [1.0, 12.0]\n"
        "param float mix = 1.0 [0.0, 1.0]\n\n"
        "x = in * drive\n"
        "y = (x + 1.0 + 400.0) % 4.0\n"
        "folded = abs(y - 2.0) - 1.0\n\n"
        "out = in * (1.0 - mix) + folded * mix\n" }
   };
   return kPresets;
}
