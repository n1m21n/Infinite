...
const std::vector<FieldElementNode::Preset>& FieldElementNode::Presets()
{
   static const std::vector<Preset> kPresets = {
      ...
      { "Proximity Vertex Heatmap",
        ...
        "publish = avgHeat\n" },
      { "Voxel Snap",
        "param float cell = 0.15 [0.02, 0.6]\n"
        "snapped = floor(P / cell + 0.5) * cell\n"
        "P = snapped\n"
        "Cd = vec3(0.3 + 0.5 * sin(snapped.x * 5.0), 0.6, 0.3 + 0.5 * sin(snapped.z * 5.0))\n"
        "output frame float cellSize = cell\n"
        "publish = cellSize\n" },
      { "Kaleidoscope Fold",
        "param float wedges = 6.0 [2.0, 16.0]\n"
        "param float spin = 0.4 [0.0, 4.0]\n"
        "px = P.x\n"
        "pz = P.z\n"
        "r = length(vec2(px, pz))\n"
        "ang = atan2(pz, px) + t * spin\n"
        "seg = 6.2831853 / wedges\n"
        "a2 = ang % seg\n"
        "a2 = abs(a2 - seg * 0.5)\n"
        "P.x = cos(a2) * r\n"
        "P.z = sin(a2) * r\n"
        "Cd = vec3(0.5 + 0.5 * sin(ang * wedges), 0.4, 0.9 - 0.4 * sin(ang * wedges))\n"
        "output frame float wedgeCount = wedges\n"
        "publish = wedgeCount\n" },
      { "Curl Noise Flow",
        "param float scale = 2.0 [0.2, 8.0]\n"
        "param float speed = 0.6 [0.0, 4.0]\n"
        "param float amp = 0.2 [0.0, 1.0]\n"
        "eps = 0.05\n"
        "n1 = sin(P.x * scale + P.z * scale * 0.7 + t * speed)\n"
        "n2 = sin((P.x + eps) * scale + P.z * scale * 0.7 + t * speed)\n"
        "n3 = sin(P.x * scale + (P.z + eps) * scale * 0.7 + t * speed)\n"
        "dndx = (n2 - n1) / eps\n"
        "dndz = (n3 - n1) / eps\n"
        "curlX = dndz\n"
        "curlZ = 0.0 - dndx\n"
        "curlY = sin(P.y * scale - t * speed * 0.8)\n"
        "P.x += curlX * amp\n"
        "P.z += curlZ * amp\n"
        "P.y += curlY * amp * 0.5\n"
        "Cd = vec3(0.5 + 0.5 * curlX, 0.5 + 0.5 * curlY, 0.5 + 0.5 * curlZ)\n"
        "flowMagLocal = abs(curlX) + abs(curlZ)\n"
        "output frame float flowMag = reduce.max(flowMagLocal)\n"
        "publish = flowMag\n" },
      { "Wind Sway",
        "param float minY = -1.0 [-3.0, 1.0]\n"
        "param float maxY = 1.0 [-1.0, 3.0]\n"
        "param float strength = 0.35 [0.0, 1.5]\n"
        "param float speed = 1.5 [0.0, 6.0]\n"
        "param float gust = 0.4 [0.0, 2.0]\n"
        "h = clamp((P.y - minY) / max(maxY - minY, 0.001), 0.0, 1.0)\n"
        "bend = h * h\n"
        "sway = sin(t * speed + P.x * 1.3) * strength\n"
        "flutter = sin(t * speed * 3.7 + P.z * 4.0) * gust * 0.15\n"
        "P.x += (sway + flutter) * bend\n"
        "Cd = vec3(0.3 + 0.5 * h, 0.6 + 0.3 * h, 0.25)\n"
        "swayMagLocal = abs(sway)\n"
        "output frame float swayMag = reduce.max(swayMagLocal)\n"
        "publish = swayMag\n" },
      { "Spike Extrude",
        "param float threshold = 0.6 [0.0, 1.0]\n"
        "param float height = 0.4 [0.0, 1.5]\n"
        "param float density = 3.0 [0.5, 10.0]\n"
        "n = rand(0.0, 1.0, density, i)\n"
        "mask = if(n > threshold, 1.0, 0.0)\n"
        "grow = clamp((n - threshold) / max(1.0 - threshold, 0.001), 0.0, 1.0)\n"
        "P += N * grow * height * mask\n"
        "Cd = mix(vec3(0.2, 0.3, 0.4), vec3(1.0, 0.3, 0.1), mask)\n"
        "output frame float spikeCount = reduce.mean(mask)\n"
        "publish = spikeCount\n" },
      { "Radial Explode",
        "param float force = 1.2 [0.0, 4.0]\n"
        "param float gravity = 1.5 [0.0, 6.0]\n"
        "param float spin = 1.0 [0.0, 5.0]\n"
        "state vec3 dir = vec3(0, 0, 0)\n"
        "state vec3 start = vec3(0, 0, 0)\n"
        "first = 1.0 - step(0.5, age)\n"
        "dir = mix(dir, normalize(P + vec3(0.0001, 0.0002, 0.0001)), first)\n"
        "start = mix(start, P, first)\n"
        "travel = age * force\n"
        "fall = age * age * gravity * 0.2\n"
        "pos = start + dir * travel\n"
        "pos.y -= fall\n"
        "ang = age * spin\n"
        "cosA = cos(ang)\n"
        "sinA = sin(ang)\n"
        "rx = pos.x * cosA - pos.z * sinA\n"
        "rz = pos.x * sinA + pos.z * cosA\n"
        "P.x = rx\n"
        "P.y = pos.y\n"
        "P.z = rz\n"
        "Cd = vec3(1.0, 0.5 + 0.5 * sin(age * 4.0), 0.15)\n"
        "output frame float maxTravel = travel\n"
        "publish = maxTravel\n" }
   };
   return kPresets;
}
