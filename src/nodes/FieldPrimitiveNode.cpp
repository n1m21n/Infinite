...
const std::vector<FieldPrimitiveNode::Preset>& FieldPrimitiveNode::Presets()
{
   static const std::vector<Preset> kPresets = {
      ...
      { "Quartz Crystal Cluster",
        ...
        "param float speed = 1.0 [0.0, 5.0]\n"
        "facets = 6.0\n"
        "sector = 6.2831853 / facets\n"
        "half = sector * 0.5\n"
        "apothem = cos(half)\n"
        "phi = uv.x * 6.2831853\n"
        "v = uv.y\n"
        "facetAngle = floor(phi / sector + 0.5) * sector\n"
        "faceScale = apothem / cos(phi - facetAngle)\n"
        "r = radius * faceScale\n"
      ...
      { "Sci-Fi Spaceship Fighter",
        ...
        "cosP = cos(phi)\n"
        "sinP = sin(phi)\n"
        "noseTaper = if(u < 0.0, pow(1.0 + u, 1.7), pow(1.0 - u, 1.15))\n"
        "hullR = noseTaper * 0.42\n"
        "wingCenter = 0.05\n"
        "wingHalfWidth = 0.42\n"
        "wingLon = clamp(1.0 - abs(u - wingCenter) / wingHalfWidth, 0.0, 1.0)\n"
        "wingAngular = pow(abs(cosP), 3.0)\n"
        "wingSpread = wingLon * wingAngular * wingSpan\n"
        "finMask = smoothstep(0.55, 0.75, u) * smoothstep(1.0, 0.85, u) * smoothstep(0.55, 0.85, sinP)\n"
        "finHeight = finMask * 0.5\n"
        "canopyMask = smoothstep(-0.05, 0.15, u) * smoothstep(0.55, 0.25, u) * smoothstep(0.15, 0.55, sinP)\n"
        "r = hullR + wingSpread\n"
        "x = r * cosP\n"
        "y = hullR * sinP * 0.6 + canopyMask * 0.22 + finHeight\n"
        "z = u * (lengthHull * 0.5)\n"
        "P = vec3(x, y, z)\n"
        "N = normalize(vec3(cosP, sinP * 1.5 + finMask * 2.0, -u * 0.8))\n"
        "engineMask = smoothstep(0.68, 0.82, u) * smoothstep(0.98, 0.82, u) * pow(abs(sinP), 2.0)\n"
        "hullColor = vec3(0.2, 0.25, 0.32)\n"
        "canopyColor = vec3(0.1, 0.8, 1.0)\n"
        "engineColor = vec3(1.0, 0.45, 0.1) * (0.6 + 0.4 * sin(t * speed * 8.0))\n"
        "Cd = mix(mix(hullColor, canopyColor, canopyMask), engineColor, engineMask)\n"
        "publish = sin((t + 1.0) * speed)\n" },
      { "Castle Turret Tower",
        PrimitiveTopology::Cylinder,
        2500,
      ...
      { "Brilliant Cut Diamond",
        ...
        "param float tableSize = 0.55 [0.2, 0.9]\n"
        "param float speed = 1.0 [0.0, 5.0]\n"
        "facets = 8.0\n"
        "sector = 6.2831853 / facets\n"
        "half = sector * 0.5\n"
        ...
        "phi = uv.x * 6.2831853\n"
        "v = uv.y\n"
        "facet = floor(phi / sector + 0.5) * sector\n"
        "faceScale = apothem / cos(phi - facet)\n"
        "girdleV = 0.62\n"
        "crownEndV = 0.84\n"
        "isPavilion = if(v < girdleV, 1.0, 0.0)\n"
        "isCrownSlope = if(v >= girdleV && v < crownEndV, 1.0, 0.0)\n"
        "isTable = if(v >= crownEndV, 1.0, 0.0)\n"
        "rBase = if(isPavilion > 0.5, radius * (v / girdleV), if(isCrownSlope > 0.5, radius - (radius - radius * tableSize) * (v - girdleV) / (crownEndV - girdleV), radius * tableSize))\n"
        "r = rBase * faceScale\n"
        "y = (v - 0.5) * height\n"
        "x = r * cos(phi)\n"
        "z = r * sin(phi)\n"
        "P = vec3(x, y, z)\n"
        "pavilionSlope = radius / (girdleV * height)\n"
        "crownSlope = (radius - radius * tableSize) / ((crownEndV - girdleV) * height)\n"
        "facetOutward = vec3(cos(facet) * apothem, 0.0, sin(facet) * apothem)\n"
        "pavilionN = normalize(facetOutward - vec3(0.0, -pavilionSlope, 0.0))\n"
        "crownN = normalize(facetOutward + vec3(0.0, -crownSlope, 0.0))\n"
        "tableN = vec3(0.0, 1.0, 0.0)\n"
        "N = normalize(pavilionN * isPavilion + crownN * isCrownSlope + tableN * isTable)\n"
        "facetShade = 0.55 + 0.45 * abs(cos(phi - facet))\n"
        "sparkle = abs(sin(facet * 4.0 + t * speed))\n"
        "Cd = vec3(0.65 + 0.35 * sparkle, 0.8 + 0.15 * facetShade, 1.0) * (0.7 + 0.3 * facetShade)\n"
        "publish = sin((t + 1.0) * speed)\n" },
      { "Bonsai Foliage Canopy",
        ...
        "param float puffiness = 0.55 [0.2, 1.0]\n"
        "param float speed = 1.0 [0.0, 5.0]\n"
        "goldenAngle = 2.3999632\n"
        "clusterCount = max(clusters, 1.0)\n"
        "u = i / count\n"
        "clusterF = floor(u * clusterCount)\n"
        "clusterU = (clusterF + 0.5) / clusterCount\n"
        "localU = fract(u * clusterCount)\n"
        "minY = -0.15\n"
        "cy = 1.0 - clusterU * (1.0 - minY)\n"
        "rAtCy = sqrt(max(0.0, 1.0 - cy * cy))\n"
        "cPhi = goldenAngle * clusterF\n"
        "cx = cos(cPhi) * rAtCy\n"
        "cz = sin(cPhi) * rAtCy\n"
        "clusterCenter = vec3(cx, cy * 0.6, cz) * spread\n"
        "puffRadius = spread * puffiness * 0.4\n"
        "py = 1.0 - localU * 2.0\n"
        "rAtY = sqrt(max(0.0, 1.0 - py * py))\n"
        "pTheta = goldenAngle * i\n"
        "localPos = vec3(cos(pTheta) * rAtY, py, sin(pTheta) * rAtY) * puffRadius\n"
        "breathe = 1.0 + 0.04 * sin((t + 1.0) * speed)\n"
        "P = (clusterCenter + localPos) * breathe\n"
        "N = normalize(localPos)\n"
        "shade = 0.5 + 0.5 * py\n"
        "Cd = vec3(0.08 + 0.18 * shade, 0.5 + 0.32 * shade, 0.16 + 0.08 * shade)\n"
        "publish = sin((t + 1.0) * speed)\n" }
   };
   return kPresets;
}
