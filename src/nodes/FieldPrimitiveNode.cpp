...
const std::vector<FieldPrimitiveNode::Preset>& FieldPrimitiveNode::Presets()
{
   static const std::vector<Preset> kPresets = {
      ...
      { "Brilliant Cut Diamond",
        ...
        "z = r * sin(phi)\n"
        "P = vec3(x, y, z)\n"
        "N = normalize(vec3(cos(facet) * (1.0 - isTable), if(isPavilion > 0.5, -0.85, if(isTable > 0.5, 1.0, 0.35)), sin(facet) * (1.0 - isTable)))\n"
      ...
      { "Bonsai Foliage Canopy",
        ...
        "phi = uv.x * 6.2831853\n"
        "theta = (uv.y - 0.5) * 3.14159265\n"
        "lobeA = sin(phi * clusters) * cos(theta * clusters * 1.15)\n"
        "lobeB = sin(phi * clusters * 1.7 + 1.3) * cos(theta * clusters * 1.6 + 0.7)\n"
        "raw = lobeA * 0.65 + lobeB * 0.35\n"
        "cloud = sign(raw) * pow(abs(raw), puffiness) * 0.4\n"
        "padShape = pow(max(cos(theta), 0.05), 0.6)\n"
        "breathe = 1.0 + 0.05 * sin((t + 1.0) * speed)\n"
        "r = spread * padShape * (1.0 + cloud) * breathe\n"
