...
const Mesh& ImageToPointsNode::GetMesh()
{
   ...
   // one here as a per-point billboard-quad mesh so a mesh:any-consuming node
   // (Join, GeometryOp, ...) would silently see *something* rather than
   // nothing. That fabrication is the same D5 mechanism fixed for MeshToPoints/
   // DistributeOnFaces/DistributeInGrid, so the honest answer - empty - is
   // correct here too. Render3D and NodeViewport already fall back to
   // GetPointCloud() whenever GetMesh() is empty, so the on-screen preview is
   // unaffected; only feeding this into a mesh-shaped operator changes, from
   // silently-wrong to correctly-empty.
...
{
   // GetMesh() is now a constant empty mesh (see D5 comment above), so there
   // is nothing to revision.
   return 0;
}
