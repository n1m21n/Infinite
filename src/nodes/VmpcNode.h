...
// modulator), or with the note input (note = base note + pad, 36..51).
//
// The output is ONE image: the clip of the last pad hit (monophonic, like a
// launcher column). With nothing playing it is transparent, or the last
// frame when `holdLastFrame` is on. Clips are silent here (use a Video node
// for a clip's soundtrack).
//
// Every pad keeps its decoder open once loaded, so a hit only seeks - no
// file open on the trigger. Notes arrive on the audio thread (that is where
// note cables are routed); a tiny AudioNode forwards them to the main
// thread through a lock-free ring, drained every frame.
class VmpcNode : public INode
{
public:
...
private:
   ...
   Pad mPads[kPads];
   std::unique_ptr<AudioVmpcNoteTap> mNoteTap;
   unsigned int mTex = 0;
