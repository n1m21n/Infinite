...
namespace
{
   ...
   // child goes idleTimeout without writing anything. Linux equivalent of
   // PluginVST3Win.cpp's DrainChildStdout (PeekNamedPipe + ReadFile) using
   // read() on an O_NONBLOCK fd instead. The timeout is an idle one, not a
   // total one: the batch scanner prints a line per bundle, so a scan of 100
   // slow-to-load plugins keeps making progress, while one plugin hanging (a
   // licence check, an invisible dialog) still gets caught.
   DrainResult DrainChildStdout(int readFd, pid_t childPid, std::chrono::seconds idleTimeout)
   {
      DrainResult result;
                       ...
                       bool& outCleanExit)
   {
      ...
      DrainResult drained = DrainChildStdout(readFd, pid, timeout);
      ::close(readFd);
   ...
   void ProbeVST3BundlesBatch(std::vector<std::string> bundlesToScan, std::vector<Platform::PluginDesc>& out)
   {
      ...
      while (!bundlesToScan.empty())
      {
         ...
         // One child per chunk. ARG_MAX (typically 2 MB, shared with the
         ...
         // whole batch, so a folder of slow-to-initialise plugins timed out
