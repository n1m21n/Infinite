...
namespace Platform
{
         ...
         while (!dirStack.empty())
         {
            ...
            while (it != end)
            {
               ...
               if (entry.is_directory(entryEc) && !entryEc)
               {
                  ...
                  {
                     DescribeVST3Bundle(entry.path().string(), out);
                  }
                  else
                  {
   ...
   PluginHandle* PluginVST3Create(const PluginDesc& desc, double sampleRate, int maxBlockFrames)
   {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
         ...
         if (bundlePath.empty())
         {
            // Direct probe in standard folders (plus any user-added VST3
            // folders) for <name>.vst3 before scanning everything.
            std::vector<std::string> probeRoots = {
               "/Library/Audio/Plug-Ins/VST3"
            };
            const char* home = getenv("HOME");
            if (home != nullptr)
               probeRoots.push_back(std::string(home) + "/Library/Audio/Plug-Ins/VST3");
            for (const auto& extra : GetExtraVST3SearchFolders())
               if (std::find(probeRoots.begin(), probeRoots.end(), extra) == probeRoots.end())
                  probeRoots.push_back(extra);

            for (const auto& root : probeRoots)
            {
               std::string candidate = root + "/" + desc.name + ".vst3";
               if (fs::exists(candidate))
               {
                  std::vector<PluginDesc> descs;
                  if (DescribeVST3Bundle(candidate, descs))
                  {
                     for (const auto& d : descs)
                     {
                        if (d.identifier == desc.identifier)
                        {
         ...
         if (bundlePath.empty())
         {
            // Search standard and user paths if not cached or directly found
            ...
            VST3Trace("  probe missed; full scan of %d folder(s)", (int)searchFolders.size());
            for (const auto& f : searchFolders)
               VST3Trace("    folder: %s", f.c_str());
            EnumerateVST3Plugins(searchFolders, discovered);
            bundlePath = GetCachedVST3BundlePath(desc.identifier);
            VST3Trace("  scan found %d plugin(s); cache lookup -> '%s'", (int)discovered.size(),
                      bundlePath.c_str());
         }
         ...
         {
            {
               ...
               v->arrivedError = "VST3 bundle not found on disk";
            }
            v->arrived.store(true, std::memory_order_release);
            return;
         }
