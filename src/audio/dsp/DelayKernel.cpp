...
void DelayKernel::ProcessBlock(const AudioBuffer& in, const AudioBuffer* /*sidechain*/, AudioBuffer& out)
{
   ...
   for (int i = 0; i < out.numFrames; i++)
   {
      ...
      const float delaySamples = mDelaySmoother.Process(targetSamples);
      ...
      if (bounce)
      {
         // Cross-feeding pair: input feeds line A, A's feedback path feeds
         // into B, B's feedback path feeds back into A - the classic
         // bounce-across-the-stereo-field topology.
