...
// EQ's kernel: 5 fixed bands in series, each one of 5 single-stage RBJ
// biquad types (low shelf, peak, high shelf, hp 12, lp 12). See
// docs/plans/audio/eq-node-prompt.md - this deliberately does NOT reuse or
// modify AudioFilterKernel/AudioFilterDsp; the two share the RBJ cookbook
// math (DspMath::Biquad) but are separate nodes with separate tables.
namespace EqDsp
{
   enum BandType
   {
...
// AudioEffectNode's kernel for the EQ node: 5 bands in series, each a single
// RBJ biquad. See IEffectKernel.h for the PushParams/ProcessBlock split this
// obeys - PushParams (main thread) computes coefficients, ProcessBlock
// (audio thread) only ever applies them.
...
public:
   ...
   void ProcessBlock(const AudioBuffer& in, const AudioBuffer* /*sidechain*/, AudioBuffer& out) override
   {
      ...
      {
         float coeffs[kNumBands][kCoeffsPerBand];
         for (int b = 0; b < kNumBands; b++)
            for (int c = 0; c < kCoeffsPerBand; c++)
               coeffs[b][c] = mMailbox.SmoothedValue(b * kCoeffsPerBand + c);
